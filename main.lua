...
local sl2 = 10
local sl3 = 10.3
local icon = love.graphics.newImage( "lovelogo.png" )
local tab_bar_idx = 1
local check1 = true
local check2 = false
local rb_idx = 1
...
local function DrawMyCustomWidget( tex, held, hovered, mx, my )
	if held then
		amplitude = (75 * my) / 150
...
function love.draw()
	...
	progress.adder = progress.adder + (10 * love.timer.getDelta())
	...
	local tex, clicked, held, released, hovered, mx, my = UI2D.CustomWidget( "widget1", 250, 150 )
	DrawMyCustomWidget( tex, held, hovered, mx, my )
	UI2D.End( pass )
